...
\textbf{Review and approval of internship advertisements by Course Co-ordinator
(Section 4.2) :}
\begin{itemize}
...
\UseCaseConditions{Post: The advert is then stored, pending approval by the CC.}
\UseCaseNonFunctionalRequirements{Security}
\UseCaseScenarios{A company logs in to the system, fills out the submission form and submits it to the system to be reviewd by the course coordinator.}
\UseCaseRisks{}
\end{UseCaseTemplate}
...
more information about it or remove irrelevant/incorrect details that may prevent the advert from being approved.}
\UseCasePriority{Must Have}
\UseCaseStatus{Not implemented}
\UseCaseActors{Company}
\UseCaseIncludes{Login, Logout}
...
\UseCaseScenarios{A company logs in, they select the option to edit their advert and select which section to edit. The appropriate changes are made and the advert remains pending approval.}
\UseCaseRisks{}
\end{UseCaseTemplate}

\subsection{Review and approval of internship advertisements by Course
Coordinator}
