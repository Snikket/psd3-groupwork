...
\textbf{TO DO!}\\
\textit{Give an overview of the system here, in the context of the surrounding
environment. Use case diagrams can be used to illustrate the
interactions between actors in the environment and the system.}
\textit{You should explain the assumptions you have made in defining the
boundary of the system (i.e. what the system will and will not do).}
\textit{Describe any conflicts in requirements expressed by different
...
\includegraphics
[scale=0.7]
{C:/Users/Mamas Nicolaou/Repositories/psd3-groupwork/Deliverables/D3/UML_D3.jpg}
\caption{Domain model}
\label{fig1:domain model}
\end{center}
\end{figure}
\textit{Explain the elements of the domain here.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\includegraphics
[scale=0.7]
{C:/Users/Administrator/company.jpeg}
\caption{Use Case Diagram 4.1}
\label{fig2:Use Case Diagram4.1}
\end{center}
\end{figure}
\begin{UseCaseTemplate}
\UseCaseLabel{Submit application}
\UseCaseDescription{A company should login and post internship adverts to the system,
confirm it and logout.}
\UseCaseRationale{A company must be able to post their internship adverts on the system in
order for students to apply for them, if suitable.}
\UseCasePriority{Must have}
\UseCaseStatus{Not implemented}
\UseCaseActors{Company}
\UseCaseIncludes{Login/Logout}
\UseCaseConditions{ Post: Course coordinator needs to review adverts to determine whether or
not it is suitable for students.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{}
\UseCaseRisks{Company may post internship adverts that may not be suitable for students.}
\end{UseCaseTemplate}
\begin{UseCaseTemplate}
\UseCaseLabel{Edit adverts submitted to the system}
...
\end{UseCaseTemplate}
\begin{UseCaseTemplate}
\UseCaseLabel{Remove adverts from the system}
\UseCaseDescription{A Company should login, select an option to remove their advert from the
system, confirm this and then logout.}
\UseCaseRationale{A company should be able to remove their advert from the system if the
position has been taken and/or the internship is no longer available.}
\UseCasePriority{Must Have}
\UseCaseStatus{Not Implemented}
\UseCaseActors{Company}
\UseCaseIncludes{Login/Logout}
\UseCaseConditions{Post: Advert should no longer be available on the system.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{}
\UseCaseRisks{Company may remove the wrong advert. Advert may not be successfully
removed.}
\end{UseCaseTemplate}
...
\includegraphics
[scale=0.7]
{C:/Users/Administrator/CourseCoordinator.jpeg}
\caption{Use Case Diagram 4.2}
\label{fig2:Use Case Diagram4.2}
\end{center}
\end{figure}
\begin{UseCaseTemplate}
\UseCaseLabel{Review company internship adverts}
...
\includegraphics
[scale=0.7]
{C:/Users/Administrator/Student.jpeg}
\caption{Use Case Diagram 4.3}
\label{fig2:Use Case Diagram4.3}
\end{center}
\end{figure}
\begin{UseCaseTemplate}
\UseCaseLabel{Review Advert List}
...
\UseCaseIncludes{Login/Logout, Apply}
\UseCaseConditions{Pre: adverts have been approved by the CC. Post: student can apply for
internship.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{}
\UseCaseRisks{}
\end{UseCaseTemplate}
\begin{UseCaseTemplate}
\UseCaseLabel{Apply for Adverts}
...
\UseCaseRationale{Student must be able to see all details for the advert, including an email
address and/or website in order to apply for the placement.}
\UseCasePriority{Must Have}
\UseCaseStatus{Not Implemented}
\UseCaseActors{Student}
\UseCaseIncludes{Login/Logout}
...
\end{UseCaseTemplate}
\begin{UseCaseTemplate}
\UseCaseLabel{Flag Inappropriate Advert}
\UseCaseDescription{Let course coordinator know an advert is not appropriate for its current
category (if, for example it is categorised as SE suitable but it shouldn’t be).}
\UseCaseRationale{A student can let the Course Coordinator know there is something wrong
with an advert on the system.}
\UseCasePriority{Could have.}
\UseCaseStatus{Not Implemented}
\UseCaseActors{Student}
\UseCaseIncludes{Login/Logout}
\UseCaseConditions{Pre: The inappropriate advert must be still visible on the system.Post:
Course Coordinator is notified.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{Student logs in to the system, finds an advert that is misplaced or does not
fulfil the School of Computing Science placement guidelines and flags it.}
\UseCaseRisks{Student might abuse this functionality by flagging random adverts .}
\end{UseCaseTemplate}
...
\includegraphics
[scale=0.7]
{C:/Users/Administrator/Section4.jpeg}
\caption{Use Case Diagram 4.4}
\label{fig2:Use Case Diagram4.4}
\end{center}
\end{figure}
\begin{UseCaseTemplate}
\UseCaseLabel{Notify Course Coordinator for Successful Application}
...
\UseCaseActors{Course coordinator}
\UseCaseIncludes{login/logout}
\UseCaseConditions{Pre: The application’s deadline for a placement has passed or a company
has confirmed an offer of position to a student and the student has confirmed he/she accepted
the position.Post: The advert is no longer visible on the system.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{}
...
\UseCaseIncludes{}
\UseCaseConditions{Post: The user is logged in if correct credentials are provided.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{}
\UseCaseRisks{}
\end{UseCaseTemplate}
...
\UseCaseActors{Company, Students, Course Coordinator}
\UseCaseIncludes{}
\UseCaseConditions{Pre: User logged in. Post: User is logged out.}
\UseCaseNonFunctionalRequirements{}
\UseCaseScenarios{}
\UseCaseRisks{}
\end{UseCaseTemplate}
