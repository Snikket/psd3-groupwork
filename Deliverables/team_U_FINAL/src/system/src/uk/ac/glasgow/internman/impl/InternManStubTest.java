...
public class InternManStubTest {
	...
	public void setUp() throws Exception {
		...
		util = new Utility();
		UserStore store=util.getStore();

		StudentImpl Bob = new StudentImpl("Smith", "Bob", studentUsername, studentPassword, "3", "Student", "test@test.com", "Computing");
		store.addUser(Bob);
	    Employer Will= new Employer(emplUsername, emplPassword,  "empl@test.com", "Will Apple");
	    store.addUser(Will);
	    User cc = new User("Storer", "Tim",ccUsername , ccPassword, "coursecoordinator");
		store.addUser(cc);
	}
	...
	public void getCurrentUserTest() {
		...
		assertTrue(interfc.getCurrentUser().getMatriculation() == studentUsername);
	}
	...
	public void getAdvertisementsForCCTest() {
		...
		for (Entry<Integer, Advertisement> entry : adds.entrySet()) {
			if (entry.getValue().getStatus()
					.equals(Advertisement.AdvertisementStatus.PENDING))
	...
	/*
	 * Testing 'getAdvertisements' meployerthod for Employer Only adverts the employer
	...
	public void getCurrentEmployerTest(){
		...
		assertTrue(emp!=null&&emp.getName().equals("emplUsername"));
	}
